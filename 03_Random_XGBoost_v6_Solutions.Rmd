...
# Load the data
data <- read_csv('https://raw.githubusercontent.com/HamidBekamiri/Econometrics-II/refs/heads/main/Data/time_series_60min_singleindex_filtered.csv')
...
df_energy <- df_energy %>%
  mutate(
    ...
    rolling_avg_4 = slide_dbl(DE_load_actual_entsoe_transparency, mean, .before = 3, .complete = TRUE),
    rolling_diff_4 = DE_load_actual_entsoe_transparency - rolling_avg_4
  ) %>%
  filter(!is.na(lag_1), !is.na(lag_2), !is.na(rolling_avg_4))
...
df_energy <- df_energy %>%
  select(year, month, day_in_week, hour, week_in_year, DE_load_actual_entsoe_transparency,
         lag_1, lag_2, rolling_avg_4, rolling_diff_4, holiday, date)%>%
  drop_na()
...
df_stock <- df_stock %>%
  mutate(
    ...
    lag_2 = lag(close, 2),
    rolling_avg_4 = slide_dbl(close, mean, .before = 3),  # 4-day rolling average
...
# Select relevant columns
df_stock <- df_stock %>%
  select(year, semester, quarter, day_in_week, week_in_year, close, lag_1, lag_2, rolling_avg_4, rolling_diff_4, date)
...
rsq_plot <- model_accuracy %>%
  ...
  geom_col(position = position_dodge(width = 0.9)) +
  geom_text(aes(label = round(rmse, 3)), vjust = -0.5, position = position_dodge(width = 0.9)) +
  labs(title = "Model Comparison - RMSE (Train vs Test)",
       x = "Model", y = "RMSE") +
  theme_minimal()
