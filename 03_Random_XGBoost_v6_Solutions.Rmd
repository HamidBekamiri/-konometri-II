...
df_passengers <- df_passengers %>% 
  mutate(
    date = as.Date(yearmonth(Month)),  # Convert Quarter to a proper date
    year = year(date),
    semester = factor(semester(date)),
    quarter = factor(quarter(date)),
    month = factor(month(date), levels = 1:12, labels = month.name)
...
model_rf <- rand_forest(trees = 500) %>%
  set_mode("regression") %>%
  set_engine("ranger") %>%
  fit(Passengers ~ ., data = training(splits))
...
model_xgb <- boost_tree(trees = 500, learn_rate = 0.05) %>%
  set_mode("regression") %>%
  set_engine("xgboost") %>%
  fit(Passengers ~ ., data = training(splits) %>% select(-date))
...
# Load the data
data <- read_csv('/Users/gy22ge/Downloads/time_series_60min_singleindex_filtered.csv')

# 1. Preprocess the data
df_energy <- data %>%
  mutate(
    date = as.Date(utc_timestamp),  # Convert Quarter to a proper date
    year = year(date),
    semester = factor(semester(date)),
    quarter = factor(quarter(date)),
    day_in_week = factor(wday(date), labels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")),
...
df_energy <- df_energy %>%
  mutate(
    lag_1 = lag(DE_load_actual_entsoe_transparency, 1),  # Lag of 1 Quarter
    lag_2 = lag(DE_load_actual_entsoe_transparency, 2),
    rolling_avg_4 = slide_dbl(DE_load_actual_entsoe_transparency, mean, .before = 3, .complete = TRUE),  # 4-quarter rolling average
    rolling_diff_4 = DE_load_actual_entsoe_transparency - rolling_avg_4  # Difference from rolling average
  ) %>%
  ungroup()
...
df_energy <- df_energy[c('year', 'semester', 'quarter', 'day_in_week', 'week_in_year', 'DE_load_actual_entsoe_transparency', 'lag_1', 'lag_2', 'rolling_avg_4', 'rolling_diff_4', 'holiday', 'date')]

df_energy <- df_energy %>%
  fill(lag_1, lag_2, rolling_avg_4, rolling_diff_4, .direction = "downup")

# 2. Split the data into training and testing sets
splits <- initial_time_split(df_energy, prop = 0.8)
...
model_fit_arima <- arima_reg() %>%
  set_engine("auto_arima") %>%
  fit(DE_load_actual_entsoe_transparency ~ date, data = training(splits)) 

# Random Forest model
model_rf <- rand_forest(trees = 500) %>%
  set_mode("regression") %>%
  set_engine("ranger") %>%
  fit(DE_load_actual_entsoe_transparency ~ . - date, data = training(splits))  # Exclude 'date' column
...
library(quantmod)
library(tseries)
indf_data <- getSymbols(Symbols = "TSLA", src = "yahoo", from = Sys.Date() - years(5), 
                        to = Sys.Date(), auto.assign = FALSE)
indf_data <- na.omit(indf_data)
chart_Series(indf_data, col = "black")
...
df_stock <- indf_data %>%
  ...
  mutate(
    year = year(date),
    semester = factor(ifelse(month(date) <= 6, 1, 2)),
    quarter = factor(quarter(date)),
    day_in_week = factor(wday(date), labels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"), levels = 1:7),
    week_in_year = factor(week(date)),
    day_in_year = factor(yday(date)),
    month = factor(month(date), levels = 1:12, labels = month.name)
  )
...
df_stock <- df_stock %>%
  mutate(
    lag_1 = lag(close, 1),  # Lag of 1 day
    lag_2 = lag(close, 2),
    rolling_avg_4 = slide_dbl(close, mean, .before = 3, .complete = TRUE),  # 4-day rolling average
    rolling_diff_4 = close - rolling_avg_4  # Difference from rolling average
  )
...
df_stock <- df_stock %>%
  select(year, semester, quarter, day_in_week, week_in_year, close, lag_1, lag_2, rolling_avg_4, rolling_diff_4, holiday, date)
...
# 2. Split the data into training and testing sets
splits <- initial_time_split(df_stock, prop = 0.8)
...
model_rf <- rand_forest(trees = 500) %>%
  set_mode("regression") %>%
  set_engine("ranger") %>%
  fit(close ~ . - date, data = training(splits))  # Exclude 'date' column
